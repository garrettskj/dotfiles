...
if [ $OS = "ubuntu" ]; then
	...
	sudo apt update
	sudo apt install htop scrot ffmpeg slack-desktop remmina atom wireshark filezilla google-chrome-beta vlc xz-utils exfat-utils net-tools xclip vim-gnome pinta tmux deluge curl minicom irssi openssh-server -y
fi
