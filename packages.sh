...
if [ $OS = "ubuntu" ]; then
	...
	sudo apt update
	sudo apt install scrot ffmpeg slack-desktop remmina atom wireshark filezilla google-chrome-beta vlc xz-utils exfat-utils net-tools xclip vim tmux deluge curl minicom irssi openssh-server -y
fi
